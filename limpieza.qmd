...
Hola a todes. Bienvenidos a este tutorial sobre procesamiento y limpieza de texto en R, en el marco del curso de Métodos Computacionales para las Ciencias Sociales de la FACSO. Si están viendo esto fuera de la asignatura, espero les sirva de igual manera. 
...
-   En la fila 1, tenemos etiquetas HTML
...
Antes de limpiar, generalmente necesitamos diagnosticar el corpus de texto que tenemos. En muchos casos es imposible hacer esto manualmente, ya que se cuenta de muchas filas (miles o cientos de miles). Para esto utilizamos `str_length` y `str_detect`.
...
`str_length` nos indica el número de carácteres de un texto. Nos puede servir para filtrar textos vacíos.
...
Esta función es muy útil para hacer filtros y categorías. Podríamos por ejemplo crear una nueva columna que nos indique que textos contienen un hashtag.
...
Podemos reemplazar un determinado patrón con la función `str_replace`. Si reemplazamos por nada, estariamos simulando el comportamiento de la función `str_remove`.
...
Si quisieramos eliminar todas las coincidencias de un patrón usamos `str_replace_all`, pero en este caso tendríamos el mismo problema. Esto ya que busca un patrón exacto. Si queremos eliminar todas las etiquetas html vamos a necesitar usar expresiones regulares.
...
Para eliminar todas las letras "e" si funciona.

## str_extract

Si quisiéramos extraer solo una parte del corpus, por ejemplo el hashtag, nos sirve esta función. Aquí viene un adelanto de expresiones regulares. Le estamos pidiendo un \# seguido de una cantidad de letras o numeros.
...
Solo extraerá la primera coicnidencia, si queremos todas se utiliza `str_extract_all`.
...
Ya sabemos que `str_replace_all(texto_1, pattern = "<p>", replacement = "")` no es suficiente, ya que tenemos `</p>`, y en el texto 3 tenemos divs, strongs y incluso un tabulador. No podemos escribir código distinto para cada caso específico que encontremos en nuestros corpus.

Lo que necesitamos es una forma de decirle a R: "No busques el texto literal '

<p>

'. Busca un patrón que se vea como una etiqueta HTML".

Para esto utilizamos las expresiones regulares.

Aquí hay un índice de algunos metacarácteres y que significan:

![](Regex-Cheat-Sheet-3362983929.png)

Lo importante no es que se los memoricen, si no que entiendan su funcionamiento general. El poder de regex nos permite en el parámetro 'pattern' especificar un rango de opciones mucho mayor que les ayudará a limpiar sus textos de forma mucho más eficaz.
...
Una forma de solucionarlo es utilizando el cuantificador peresozo, le decimos que se detenga en la primera coincidencia posible.
...
Como ven, el texto 3 ya está casi listo para analizarlo, pero queda un salto de linea. Podemos reemplazarlo con replace_all (o remove_all) por un espacio o por nada. Miren sus datos para saber esto, normalmente los saltos de línea (\n) deberán cambiarlos por un espacio.
```{r}
...
Ya vimos como limpiar un texto con stringr y regex, en diferentes situaciones, pero ¿Cómo lo hacemos en un dataframe completo? Tenemos varios enfoques.
...
Tenemos html, mayusculas y espacios por todos lados. Necesitamos aplicar una limpieza en varios pasos:
...
Esta es la práctica más transparente y legible. Iteramos por cada fila, aplicamos la función y guardamos el contenido en un vector de carácteres. Luego, asignamos ese vector a una nueva columna (contenido limpio for)
```{r}
...
Cómo ven, ambos enfoques dan el mismo resultado. Sapply por detrás es un bucle for, que itera por cada elemento y devuelve un vector. Lo más importante es la función que ustedes construyen y le entregan a sapply, en este caso, `limpiar_mi_texto`.

## Tercer enfoque: método directo

Cómo bonus, stringr ya funciona de forma vectorial (es como si fuera un sapply por detrás, pero más eficiente). Para casos más sencillos es posible que simplemente aplicando las funciones de stringr sobre la columna ya lleguen a solucionar el problema.
...
Todas estos enfoques nos sirven para títulos:
...
Llegando al final de este taller, es crucial abordar el tema del uso de Inteligencia Artificial. Esta se fue muy utilizada en sus entregas, e incluso es un objetivo pedagógico que aprendan a usarla. La IA es una herramienta muy poderosa para programar.

Sin embargo, para usar esta de forma eficaz hay seguir buenas prácticas, y no basta con copiar y pegar lo que les pasa el chatbot. De esta forma, se quedan en la "caja negra", obtienen el resultado, pero no aprenden, y si su código falla en un futuro no sabrán como arreglarlo.
...
1.  Pedirle que les expliquen lo que está haciendo, si no entienden cada uno de los pasos algo está mal. Diganle que les explique el código paso a paso y de forma sencilla.
...
3. Usenla como depurador (Debugger): ¡Es de los mejores usos! intenten avanzar lo que más puedan por su cuenta, y utilicen la Inteligencia Artificial para diagnosticar y resolver los errores que encuentren en el código.
...
2. Entendimos para que necesitamos `regex` y lo utilizamos para limpiar etiquetas de html
3. Aprendimos a automatizar la limpieza de texto en dataframes, creando una función y aplicandola a través de tres envoques: `for loop`, `sapply` y vectorización
4. Vimos como usar la IA como co-piloto en nuestro aprendizaje

La idea es que estae taller les sirva para limpiar texto independiente del contexto en el que lo necesiten realizar. El proceso de limpieza no es lineal, es iterativo. Probablemente intentarán una función, luego verán otro carácter indeseado, corregirán su función y seguirán iterando.
...
Bonus: Crea una nueva columna con los hashtag.